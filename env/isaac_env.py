...
class env:
    def __init__(self, root_path, render, physics_dt=1 / 60.0) -> None:
        ...
        # sample parameter
        # relative pose
        self.relative_pos_upper = np.array([-0.05 + 0.02, 0.05, -0.12])
        self.relative_pos_lower = np.array([-0.05 - 0.02, -0.05, -0.15])
        # self.relative_pos_upper = np.array([0.01, 0.01, 0.15])
        # self.relative_pos_lower = np.array([-0.01, -0.01, 0.1])
        self.relative_rot_upper = np.array([np.pi / 6, np.pi / 6, np.pi / 6])
        self.relative_rot_lower = np.array([-np.pi / 6, -np.pi / 6, -np.pi / 6])
        # self.relative_rot_upper = np.array([np.pi/3, np.pi/3, np.pi/3])
        # self.relative_rot_lower = np.array([-np.pi/3, -np.pi/3, -np.pi/3])

        # in_hand_error_pose
        self.hand_error_pos_upper = np.array([0.01, 0.0, 0.14])
        self.hand_error_pos_lower = np.array([-0.01, 0.0, 0.12])
        self.hand_error_rot_upper = np.array([0.0, 0.0, 0.0])
        self.hand_error_rot_lower = np.array([0.0, 0.3, 0.0])
